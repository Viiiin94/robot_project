...
본 프로젝트는 FPGA를 이용하여 **로봇의 구동부(DC 모터, 서보 모터)**,  
라즈베리파이를 이용한 **로봇의 머리(각 모터를 제어하는 전처리 값)** 그리고
...
- 라즈베리파이에서 전처리 하여 FPGA에서 계산을 최소화 하여 동작
...
본 시스템은 전처리 역할을 하는 Raspberry Pi와 동작만을 하는 FPGA를 결합하여 FSM 구조에 맞는 병렬 처리 시스템입니다.

- 각 동작부가 동작할 수 있는 패킷을 UART로 송신
- 모터 제어용 UART 데이터 수신
...
### Pre-Scaling
- 기존의 FPGA에서만 제어하는 방식에서 라즈베리파이를 이용하여 전처리
- 단순 값을 받아서 처리하는 것이 아닌 계산된 값으로 동작
- 더 나아가 파이썬의 패키지를 활용하여 영상에서 객체\(사람 얼굴, 손 모양\)를 인식하여 조작
...
- Raspberry PI 4 Model B: 전처리 가능한 OS(Linux)
- FPGA Board: (사용 보드 기입)
- HDL: Verilog HDL
...
**실제 로봇 제어 시스템에서 사용 가능한 구조를 목표로 설계**
